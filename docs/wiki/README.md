...
The public Wiki at https://github.com/dyh1265/DocuMind/wiki is **a mirror** of this folder. We edit Markdown here, version it with the rest of the code, and a GitHub Action pushes it to the Wiki on every push to `master`.
...
| `Configuration.md` | https://github.com/dyh1265/DocuMind/wiki/Configuration |
...
```markdown
See [`backend/core/pipeline.py`](https://github.com/dyh1265/DocuMind/blob/master/backend/core/pipeline.py).
```

## One-time setup (already done; here for reference)

1. **Initialise the wiki repo.** Wikis only exist after the first page is created via the GitHub UI. Go to https://github.com/dyh1265/DocuMind/wiki and click *Create the first page* — anything will do; the workflow will overwrite it.
2. **Allow Actions to push to the wiki.** Repo → Settings → Actions → General → *Workflow permissions* → set to *Read and write permissions*. The Wiki repo (`dyh1265/DocuMind.wiki.git`) accepts the default `GITHUB_TOKEN` once this is on.
...
Within ~30 seconds, the [Wiki Sync workflow](https://github.com/dyh1265/DocuMind/actions/workflows/wiki.yml) republishes the page.
...
```bash
git clone https://github.com/dyh1265/DocuMind.wiki.git
cp docs/wiki/*.md DocuMind.wiki/
