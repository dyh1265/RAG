"""Phase 2 ingest post-processing — section paths, chunking, enrichment."""
...
from phases.phase_02_long_doc_retrieval.chunking.semantic_chunker import SemanticChunker
from phases.phase_02_long_doc_retrieval.enrichment.context_enricher import doc_title_from_path, enrich_chunks
from phases.phase_02_long_doc_retrieval.enrichment.section_paths import apply_section_paths
from phases.phase_02_long_doc_retrieval.retrieval.hybrid_retriever import invalidate_bm25_cache


@dataclass
class Phase2IngestConfig:
...
def apply_phase2_ingest(
    chunks: list[DocumentChunk],
    path: Path,
    config: Phase2IngestConfig | None = None,
) -> list[DocumentChunk]:
    """Run Phase 2 transforms after Phase 1 parsing."""
    cfg = config or Phase2IngestConfig()
    result = list(chunks)
...
def invalidate_phase2_caches(doc_id: str) -> None:
    """Clear Phase 2 query-time caches after re-ingest."""
